...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.plot(df['date'], df['order_count'], label='Órdenes por día', color='blue')

    # Marcar los días festivos con líneas verticales
    for index, row in df.iterrows():
        if row['holiday']:
            plt.axvline(x=row['date'], color='red', linestyle='--', alpha=0.5, label='Día festivo')

    # Configurar el gráfico
    plt.title('Cantidad de órdenes por día con días festivos')
    plt.xlabel('Fecha')
    plt.ylabel('Cantidad de órdenes')
    plt.xticks(rotation=45)
    plt.grid(True, linestyle='--', alpha=0.7)

    # Mostrar la leyenda (evitar duplicados)
