...
def query_revenue_by_month_year(database: Engine) -> QueryResult:
    ...
    print(QueryResult(query=query_name, result=result))
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    # (Optional) Ensure that the 'date' column is of type date
    result_df["date"] = pd.to_datetime(result_df["date"]).dt.date

    # Convert the 'date' column in the holidays DataFrame to date objects.
    holidays["date"] = pd.to_datetime(holidays["date"])  # Convertir a datetime
    holiday_dates = holidays["date"].dt.date
    

    # Create a new boolean column 'holiday': True if the date is in the holidays, False otherwise.
    result_df["holiday"] = result_df["date"].isin(holiday_dates)
